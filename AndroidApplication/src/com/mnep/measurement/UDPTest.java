...
public class UDPTest {
	...
	public boolean runUDPTest() {
		try { 
			...
			for(int k =0;k <5; k++) {
				if(MNEPApplication.bDebug) Log.d(TAG, "@runUDPTest : Demo packet sent - B "+k+", socket - "+dsUDPSocket.getLocalSocketAddress()+", "+dsUDPSocket.getRemoteSocketAddress());
				dsUDPSocket.send(dpUDPSendPacket);	
				if(MNEPApplication.bDebug) Log.d(TAG, "@runUDPTest : Demo packet sent - A "+k+", socket - "+dsUDPSocket.getLocalSocketAddress()+", "+dsUDPSocket.getRemoteSocketAddress());
			}
			
			if(MNEPApplication.bDebug) Log.i(TAG, "@runUDPTest : UDP Transactions " + iUDPPackets + " packets");
			...
			for (int i = 0; i < iUDPPackets; i++){
				try{
					if(iDirection == 0) {
						...
						// if(MNEPApplication.bDebug) Log.d(TAG, "@UDPTest : C->S Packet1- " + i + " sent"+", packet delay - "+iPacketDelay+", client time - "+lClientTime);
						
						// if(MNEPApplication.bDebug) Log.d(TAG, "@UDP : C2S " + i);
					...
					if(iDirection == 1) {		
						...
						long lLatencyDownLink = lTimeOnClient - Measurement.lClientOffsetFromNTP - lTimeOnServer; // (lTimeOnClient-MNEPUtilities.lTimeDifference) - (lTimeOnServer + lOffsetDifferenceClientAndServer - Measurement.lOffsetServerAndNTP);
						lUDPPacketReceivedTimes[i] = lLatencyDownLink;
						iaUDPBytes[i] = iUDPBytesReceived;

						i = iPacketNumber;
						// if(MNEPApplication.bDebug) Log.d(TAG, "@UDPTest : S->C Packet " + iPacketNumber + " received"); //, Total bytes received - "+iUDPBytesReceivedFromServer+", stime - "+lTimeOnServer+", ctime - "+lTimeOnClient+", diff - "+(lTimeOnClient-lTimeOnServer)); // +", dpUDPPacket data - "+new String(dpUDPPacket.getData()));
						...
						// if(MNEPApplication.bDebug) Log.d(TAG, "@UDP : S2C " + iPacketNumber);
					}
				} catch(Exception e){
