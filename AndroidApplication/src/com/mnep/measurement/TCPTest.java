...
public class TCPTest {
	...
	public boolean runTCPTest() {
		...
		if(MNEPApplication.bDebug) Log.d(TAG, "@runTCPTest : Connecting to server ..");
		...
		try {
			...
			while(++iTCPConnectionRetryCount < 6) {
				try {
					...
					if(sConnectionSocket != null) {
						if(MNEPApplication.bDebug) Log.d(TAG, "@runTCPTest : Connected to Server : "+sConnectionSocket.getRemoteSocketAddress());
						break;
					}
				} catch(Exception e) {						
			...
			if(MNEPApplication.bDebug) Log.i(TAG, "@runTCPTest : TCP Transactionn - " + iTCPPackets + " Packets.");
			...
			for (int i = 0; i <iTCPPackets; i++){
				try{
					if(iDirection == 0) {
						...
						// if(MNEPApplication.bDebug) Log.d(TAG, "@runTCPTest : +"+", packet delay - "+iPacketDelay+", Sent - " + i+", Bytes count -- "+sBuffer.getBytes().length+", Total bytes sent - "+iTCPBytesSentToServer);

						// if(MNEPApplication.bDebug) Log.d(TAG, "@TCP : C2S "+i+);
					...
					if(iDirection == 1) {
						...
						// long lLatencyDownLink = (lTimeOnClient-MNEPUtilities.lTimeDifference) - (lTimeOnServer + lOffsetDifferenceClientAndServer - Measurement.lOffsetServerAndNTP);
						long lLatencyDownLink = lTimeOnClient - Measurement.lClientOffsetFromNTP - lTimeOnServer; // MNEPUtilities.lTimeDifference) - (lTimeOnServer + lOffsetDifferenceClientAndServer - Measurement.lOffsetServerAndNTP);
						
						i= iTCPPacketNumber;
						// if(MNEPApplication.bDebug) Log.d(TAG, "@TCPTest : S->C Packet " + lTimeOnServer + " received, Total bytes received - "+iTCPBytesReadFromServer+", diff - "+lLatencyDownLink+" toc "+lTimeOnClient+", tos "+lTimeOnServer+", of - "+Measurement.lClientOffsetFromNTP);
						// if(MNEPApplication.bDebug) Log.d(TAG, "@TCP : S2C "+iTCPPacketNumber);
