...
if ($queryInfo['totalRows'] > 0) {
    foreach ($loginresultset->rows() as $loginresultrow) {
        if ($loginresultrow['status'] == "1") {
            $get_status_list = $bigQuery->runQuery("SELECT COUNT(teb.transferid) AS totalExecuted, 
                                                    COUNT(ttl.transferid) AS totalTransfers
		                                            FROM MITATE.experiment exp, MITATE.transactions tr, MITATE.trans_transfer_link ttl 
		                                            LEFT OUTER JOIN MITATE.transferexecutedby teb 
            ...
            foreach ($get_status_list->rows() as $get_status) {
