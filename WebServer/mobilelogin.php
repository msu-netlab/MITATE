...
if($_GET['username']!="" || $_GET['password']!="")
{
    ...
    if ($loginresultset) {
        ...
        if ($loginresultrow['status'] == "1") {
			...
			and ((6378.137 * ACos(Cos(cast($_GET[latitude] as decimal)) * Cos(cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 1), ';', -1) as decimal)) * Cos(cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 2), ';', -1) as decimal) - cast($_GET[longitude] as decimal)) + Sin(cast($_GET[latitude] as decimal) * Sin(cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 1), ';', -1) as decimal))))) <= cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 3), ';', -1) as decimal) 
			or (6378.137 * ACos(Cos(cast($_GET[latitude] as decimal)) * Cos(cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 1), ';', -1) as decimal)) * Cos(cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 2), ';', -1) as decimal) - cast($_GET[longitude] as decimal)) + Sin(cast($_GET[latitude] as decimal) * Sin(cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 1), ';', -1) as decimal))))) <= cast(SUBSTRING_INDEX(SUBSTRING_INDEX(cri.specification, ';', 3), ';', -1) as decimal) = '')
