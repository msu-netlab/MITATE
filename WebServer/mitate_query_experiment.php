    ...
    if ($loginresultrow['status'] == "1") {
		...
		while($get_experiment = mysql_fetch_assoc($get_experiment_list)) {
			...
			while($get_transaction = mysql_fetch_assoc($get_transaction_list)) {
				echo "insert into transaction1 (transactionid, username, count, original_count, experiment_id) values($get_transaction[transactionid], '$get_transaction[username]', $get_transaction[count], $get_transaction[original_count], $get_transaction[experiment_id]);";
				...
				while($get_criteria_linked = mysql_fetch_assoc($get_criteria_linked_list)) {
					echo "insert into trans_criteria_link (criteriaid, transactionid) values ($get_criteria_linked[criteriaid], $get_criteria_linked[transactionid]);";
					...
					while($get_criteria = mysql_fetch_assoc($get_criteria_list)) {
						echo "insert into criteria (criteriaid, specification, deviceid) values($get_criteria[criteriaid], '$get_criteria[specification]', '$get_criteria[deviceid]');";
					}
				}
				...
				while($get_transfer_linked = mysql_fetch_assoc($get_transfer_linked_list)) {
					echo "insert into trans_transfer_link (transferid, transactionid, orderno) values($get_transfer_linked[transferid], $get_transfer_linked[transactionid], $get_transfer_linked[orderno]);";
